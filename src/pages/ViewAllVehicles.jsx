...
const ViewAllVehicles = () => {
  ...
  const retrieveAllVehiclesByNumber = async () => {
    const response = await axios.get(
      "http://171.228.167.35:8080:8080/api/transport/vehicle/fetch/vehicle-no-wise?vehicleNo=" +
        vehicleNumber,
      {
        headers: {
          //   Authorization: "Bearer " + admin_jwtToken, // Replace with your actual JWT token
  ...
  const deleteVehicle = (vehicleId, e) => {
    fetch(
      "http://171.228.167.35:8080:8080/api/transport/vehicle/delete?vehicleId=" +
        vehicleId,
      {
        method: "DELETE",
        headers: {
